...
StepKind = Literal["research", "analysis", "synthesis", "review", "action"]
...
class Orchestrator:
    ...
    It does not call an LLM. It decomposes goals into typed steps, assigns them
    to role-matched agents, invokes registered tools, records collaboration
    messages, and runs a final self-review pass.
    """
    ...
    def plan(self, goal: str) -> Plan:
        ...
        if self._needs_synthesis(goal) or not steps:
            agent = self._agent_for("writer", fallback_role="analyst")
            steps.append(
                TaskStep(
                    id=f"step-{len(steps) + 1}",
                    kind="synthesis",
                    instruction=f"Synthesize result for: {goal}",
    ...
    @staticmethod
    def _needs_synthesis(goal: str) -> bool:
        text = goal.lower()
